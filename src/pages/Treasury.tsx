...
export default function Treasury() {
  ...
  const currentBalance = salesTotal + insuranceTotal - purchasesTotal - mockTreasuryEntries.filter(e => e.type === 'withdrawal').reduce((s, e) => s + e.amount, 0);
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{currentBalance.toFixed(2)} ر.س</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{salesTotal.toFixed(2)} ر.س</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-card">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">مبيعات التأمين</p>
                  <p className="text-2xl font-bold text-foreground">{insuranceTotal.toFixed(2)} ر.س</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{purchasesTotal.toFixed(2)} ر.س</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="border-0 shadow-card">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button onClick={() => setShowAddDialog(true)} size="sm">
              <Plus className="h-4 w-4 ml-2" /> إضافة حركة
            </Button>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {mockTreasuryEntries.map(entry => (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell>
                      <span className={`inline-flex items-center gap-1 text-sm ${entry.type === 'income' || entry.type === 'deposit' ? 'text-success' : 'text-destructive'}`}>
                        {entry.type === 'income' || entry.type === 'deposit' ? <ArrowUpCircle className="h-4 w-4" /> : <ArrowDownCircle className="h-4 w-4" />}
                        {entry.type === 'income' ? 'دخول' : entry.type === 'expense' ? 'خروج' : entry.type === 'deposit' ? 'إيداع' : 'سحب'}
                      </span>
                    </TableCell>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div>
              <Label>النوع</Label>
              <Select value={newEntry.type} onValueChange={(v: 'deposit' | 'withdrawal') => setNewEntry({ ...newEntry, type: v })}>
            ...
            <div>
              <Label>الوصف</Label>
              <Input value={newEntry.description} onChange={e => setNewEntry({ ...newEntry, description: e.target.value })} placeholder="وصف الحركة..." />
            </div>
            <div>
              <Label>المبلغ</Label>
