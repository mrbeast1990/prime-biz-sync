    ...
    <>
      ...
      <div className="md:hidden space-y-3">
        {products.map((product) => (
          <div key={product.id} className={cn('rounded-xl bg-card p-4 shadow-card', isLowStock(product) && 'border border-destructive/30')}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                {product.image_url && <img src={product.image_url} alt={product.trade_name} className="h-10 w-10 rounded object-cover flex-shrink-0" />}
                <div className="min-w-0">
                  <p className="font-medium text-card-foreground truncate">{product.trade_name}</p>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  <DropdownMenuItem onClick={() => onEdit(product)}><Edit className="ml-2 h-4 w-4" />تعديل</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onDelete(product)} className="text-destructive focus:text-destructive"><Trash2 className="ml-2 h-4 w-4" />حذف</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
            ...
            <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {product.packaging_type && <span className="rounded-full bg-primary/10 px-2 py-0.5 text-primary">{product.packaging_type} ({product.units_per_package})</span>}
            </div>
          </div>
        ))}
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {products.map((product, index) => (
                <tr key={product.id} className={cn('border-b border-border table-row-hover', index % 2 === 0 ? 'bg-card' : 'bg-muted/20')}>
                  <td className="px-4 py-3"><span className="font-mono text-sm text-muted-foreground">{product.barcode}</span></td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {product.image_url && <img src={product.image_url} alt={product.trade_name} className="h-8 w-8 rounded object-cover" />}
                      <div>
                        <span dir="auto" className="font-medium text-card-foreground">{product.trade_name}</span>
                        {isLowStock(product) && <AlertTriangle className="inline-block mr-1 h-4 w-4 text-warning" />}
                      </div>
                    </div>
                  </td>
                  <td dir="auto" className="px-4 py-3 text-sm text-muted-foreground">{product.scientific_name}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {product.has_expiry ? (
                      <span className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium bg-muted text-muted-foreground">خاضع للصلاحية</span>
                    ) : (
